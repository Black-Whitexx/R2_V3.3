...
void RoboRunTask(void const *argument) {
    ...
    for (;;) {
        ...
        {
            if (Distance_Calc(Aim_Points[AimPoints_Index], LiDar.locx, LiDar.locy) < 0.02f &&
...
//                  CLAW_OFF;                /** 打开夹爪 **/
                    Slope_Pos = Slope_ON;
                    vTaskResume(Run1to3_TaskHandle);
//                  vTaskSuspend(NRF_TaskHandle);
                    break;
                case Right_Up:
                    cmd = 0x01;
    ...
    for (;;) {
        ...
        switch (State) {
            case Default_State:
                break;
            case Run2Get_State:
                Set_Point(&Vision_Points[0], 3.19f, 9.60f, 90, 0);
                vision_cmd = 0x02;
...
//                while (State == Run2Store_State){ osDelay(1);}
            ...
            case TakeRightBall_State:/** 取正确的�??? **/
                ...
                CLAW_ON;//关闭夹爪
                cnt = 0;
//                Set_Point(&Aim_Points[AimPoints_Index],4.50f,9.45f,90,0);
                Set_Point(&Aim_Points[AimPoints_Index], 3.19f, 9.60f, 90, 0);
//                vTaskSuspend(Vision_TaskHandle);
                vTaskResume(RoboRun_TaskHandle);
...
void Run1to3Task(void const *argument) {
    ...
    /* Infinite loop */
    for (;;) {
        if (Distance_Calc(Run1to3_Points[index], LiDar.locx, LiDar.locy) < 0.02f &&
        ...
        if (xQueueReceive(VisionData_QueueHandle, &visiondata, 0) == pdTRUE) {
            if (visiondata.flag == 1)//在黄区找到球了
            {
                ...
                Set_Point(&Vision_Points[0], -visiondata.vision_y / 1000.0f + LiDar.locx,
                          visiondata.vision_x / 1000.0f + LiDar.locy, 90, 0);//更新VisionPoint结构体
                vTaskResume(VisionRun_TaskHandle);//释放找球跑点任务
                vTaskSuspend(Vision_TaskHandle);//挂起自身
                ...
                switch ((int16_t) (visiondata.vision_y)) {
                    case 1:
                        ...
                        break;
                    case 2://无效球状态，等待无效球排出
                        printf("2,2\n");
                        Color_Flag = purple;
                        Slope_Pos = Slope_ON;
                        Toggle_Pos = Toggle_Mid;/** 爪子回中位 **/
                        omega = PID_Realise(&Turn_PID, 90, LiDar.yaw, 0.5f, 0.5f);//角度PID
                        SGW2Wheels(0, 0.2f, omega, 0);//缓慢前进
                        break;
                    case 0://跟踪目标球态
                        //TODO:加入转体动作
                        printf("2,0\n");
                        ...
                        if (Vision_State != Vision_Delay) {
                            ...
                            } else if (str_flag == 1)/** 还没对好 **/
                            {
                                Speed_x = -PID_Realise(&VisionPID_X, 0, Vision_Data.vision_x, 1.0f, 3);
                                omega = PID_Realise(&Turn_PID, 90, LiDar.yaw, 0.4f, 0.5f);
                                SGW2Wheels(Speed_x, 0, omega, 0);
                                if (fabsf(Vision_Data.vision_x) < 8.0f) {
                                    str_flag = 2;
                                }
                            } else {
                                float Angle_Ball = atanf(Vision_Data.vision_y / Vision_Data.vision_x);
                                omega = PID_Realise(&Turn_PID, 90, Angle_Ball + 90.f, 0.4f, 0.5f);
                                SGW2Wheels(0, 0, omega, 0);
                                if (fabsf(LiDar.yaw - Angle_Ball) < 1.0f) {
                                    str_flag = 1;
                                }
                            }
                            break;
                ...
                switch ((int16_t) (visiondata.vision_y)) {
                    case 6://有效球状态
                        ...
                        vTaskSuspend(VisionRun_TaskHandle);//挂起自身，准备跑点
                        break;
                    case 7://球已排出
                        if (Vision_State == Vision_Delay) {
                            OpenSuction();
...
void VisionRunTask(void const *argument) {
    ...
    /* Infinite loop */
    for (;;) {
        if (Distance_Calc(Vision_Points[0], LiDar.locx, LiDar.locy) < 0.02f &&
            fabsf(LiDar.yaw - Vision_Points[0].angle) < 1.0f) {
            cnt = 0;
            Car_Stop;
            if ((Vision_State == Vision_FindBall) || (State == Run2Get_State) ||
                State == (Run2Get_State2)) /** 从黄区到绿区的跑点，用于去找球，到点后启动5065，开启视觉任务 **/
            {
...
void StartTask(void const *argument) {
    ...
    PointStruct Start_Points[2] = {
            {.x = 1.69f, .y = 9.54f, .angle = 90.0f, .num = 0},
            {.x = 3.78f, .y = 8.06f, .angle = 90.0f, .num = 0}
    };
    /* Infinite loop */
    for (;;) {
        if (Distance_Calc(Start_Points[index], LiDar.locx, LiDar.locy) < 0.02f &&
            fabsf(LiDar.yaw - Start_Points[index].angle) < 0.5f) {
            index++;
            cnt = 0;
            if (index == 1) {
                Car_Stop;    /** 停车 **/
...
//          printf("%f,%f,%f,%f\n",Distance_Calc(DT32_Points,DT35_Data.forward,DT35_Data.Right),DT35_Data.forward,DT35_Data.Right,LiDar.yaw);
